...
# Эта функция ниже создана для создания самого первого блока и установки его хэша равным "0"
    ...
    def postgre ():
        conn = psycopg2.connect(dbname = 'postgres', user='postgres', password='666666', host='localhost')
        cursor = conn.cursor()
...
# Эта функция ниже создана для добавления дополнительных блоков в цепочку
    def create_block(self, proof, previous_hash):
        ...
        block = {'index': len(self.chain) + 1,
                 ...
                 'id': str(df[1].iloc[len(self.chain)]),
                 'adress': str(df[2].iloc[len(self.chain)]),
                 'start_currency': str(df[3].iloc[len(self.chain)]),
                 'end_currency': str(df[4].iloc[len(self.chain)]),
                 'start_value': str(df[4].iloc[len(self.chain)]),
                 'end_value': str(df[4].iloc[len(self.chain)]),
                 'proof': proof,
